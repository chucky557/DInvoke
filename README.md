...
> Note: This project is still under development and most of the things will be left as is until fully translated to nim or nim compliant documentation.

Features:
* Dynamically invoke unmanaged APIs 
...
#=================>
"""
stdout.write(testDetail)

#Get NTDLL base from the PEB
stdout.write("[?] Resolve Ntdll base from the PEB..")
var hNtdll: pointer = GetPebLdrModuleEntry("ntdll.dll")
stdout.write(fmt"[>] Ntdll base address : {addr hNtdll}")

# Search function by name
stdout.write("[?] Specifying the name of a DLL (\"ntdll.dll\"), resolve a function by walking the export table in-memory..")
stdout.write("[+] Search by name --> NtCommitComplete")
var pNtCommitComplete: pointer = GetLibraryAddress("ntdll.dll", "NtCommitComplete", true)
stdout.write(fmt"[>] pNtCommitComplete : 0x{pNtCommitComplete}")

stdout.write("[+] Search by ordinal --> 0x260 (NtSetSystemTime)")
var pNtSetSystemTime: pointer = GetLibraryAddress("ntdll.dll", 0x260, true)
stdout.write(fmt"[>] pNtSetSystemTime : {addr pNtSetSystemTime}")

stdout.write("[+] Search by keyed hash --> 138F2374EC295F225BD918F7D8058316 (RtlAdjustPrivilege)")
stdout.write("[>] Hash : HMACMD5(Key).ComputeHash(FunctionName)")
var fHash: string = GetAPIHash("RtlAdjustPrivilege", 0xaabb1122)
var pRtlAdjustPrivilege: pointer = GetLibraryAddress("ntdll.dll", fHash, 0xaabb1122)
stdout.write(fmt"[>] pRtlAdjustPrivilege : {addr pRtlAdjustPrivilege}")

# Search for function from base address of DLL
stdout.write(fmt"[?] Specifying the base address of DLL in memory ({hNtdll}), resolve function by walking its export table...")
stdout.write("[+] Search by name --> NtCommitComplete")
var pNtCommitComplete2: pointer = GetExportAddress(hNtdll, "NtCommitComplete")
stdout.write(fmt"[>] pNtCommitComplete : {addr pNtCommitComplete2}")

# Pause execution
stdout.write("[*] Pausing execution..")
